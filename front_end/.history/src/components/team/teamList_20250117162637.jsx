...
const TeamList = () => {
  ...
  const createTeam = async () => {
    try {
      const response = await axiosInstance.post('https://work-hive.liara.run/api/teams', { name: teamName });
      ...
      <div className="space-y-4">
        {teams.map((team) => (
          <div key={team.id} className="p-4 border rounded-lg shadow-md bg-white">
            ...
            <p className="text-sm text-gray-500">{team.users.length} Members</p>
          </div>
        ))}
      </div>
