...
function NewProject() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('/api/projects', { name, teamId });
            if (response.status === 201) {
                navigate('/projects'); // Navigate to the projects page on success
            }
        } catch (error) {
            // Check for validation or server errors
